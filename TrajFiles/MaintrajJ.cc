...
////////////////////// Input tests //////////////////////
	...
	commonelectrontraj.ClusterBackDetMC=myconfig.pBool("ClusterBackDetMC");
	commonelectrontraj.Conductor=myconfig.pString("Conductor");
	...
	if(commonelectrontraj.ClusterBackDetMC && commonelectrontraj.ClusterMC){
		cout<<"Please check the values for Cluster MC and Cluster Back DetMC, only 1 can be true"<<endl;
		return 0;
	}
	else if(commonelectrontraj.ClusterBackDetMC && commonelectrontraj.ClusterBackMC){
		cout<<"Please check the values for Cluster Back MC and Cluster Back DetMC, only 1 can be true"<<endl;
		return 0;
	}
	else if(commonelectrontraj.ClusterMC && commonelectrontraj.ClusterBackMC){
		cout<<"Please check the values for Cluster MC and Cluster Back DetMC, only 1 can be true"<<endl;
		return 0;
	}
...
*/      
	MonteCarloOut << "BRxB" << "\t";
	MonteCarloOut << "xDet" << "\t" << "yDet" << "\t" << "zDet" << "\t" << "DetPhase" << "\t" << "DetB" << "\t" << "ThetaDet" <<"ParticleNo"<< endl; //relative to the z axis
	...
	else thetamax_rad=45/180.*M_PI; //to be edited out later
...
//buffer1 is the proton energy, swotch it around with the Ekin in the while condition 
	counter = 0;
	//while ( MonteCarloData >> commonelectrontraj.thetaStartmax >> commonelectrontraj.Startphi >> commonelectrontraj.Ekin >> buffer1  ){
	while(MonteCarloData >> commonelectrontraj.thetaStartmax >> commonelectrontraj.Startphi >> commonelectrontraj.Ekin >> commonelectrontraj.Xstart >> commonelectrontraj.Ystart>>commonelectrontraj.Zstart>>commonelectrontraj.counter){
	    //cout << endl << "TRAJ: Decay#: " << counter << endl;	    
	    counter ++;
	
		commonelectrontraj.thetaStartmax=180+commonelectrontraj.thetaStartmax;
		commonelectrontraj.Startphi = 180+commonelectrontraj.Startphi;

	    // if theta is greater than 45°, filter will reflect -> cutoff
	    //if( commonelectrontraj.thetaStartmax <= 45. ){
		//if( commonelectrontraj.thetaStartmax > 80. ){
		...
		//cout<<"Gyration Radii of "<<counter<<" is "<<StartgyraR<<endl;
		...
		//Ended coments here
		commonelectrontraj.Zstart= -0.3;
		//commonelectrontraj.zdetectr=commonelectrontraj.Zstart-0.001; //to be edited later
		//commonelectrontraj.Ystart= commonelectrontraj.R_1 + apertYshift + (ApertY+ApertExpander)/2. * dist(eng); //dist(eng) gives numb between -1 and 1 for selection of aperture position
		//commonelectrontraj.Xstart= apertXshift+ (ApertX+ApertExpander)/2. * dist(eng); 
...
//------------------------------------------------------------------------//
	 if( commonelectrontraj.ClusterBackMC ){
        ...
        cout << endl << "TRAJ: ClusterBackMC run" << endl << endl;
...
*/      
	...
	DVwindow_shiftX = apertXshift * sqrt_r_A;
	DVwindow_shiftY = apertYshift * sqrt_r_A;
	cout << "N Value is"<<n[3]<<" "<<n[2]<<" "<<n[1]<<endl;
	//calc max gyration radius from 45° and maximum energy
	if ( particle == "e" ){
		velocity(n,781585.,cos(thetamax_rad),0.,velo); 
	}
...
//buffer1 is the proton energy, swotch it around with the Ekin in the while condition 
	counter = 0;
	commonelectrontraj.counter=0;
	while ( MonteCarloData >> commonelectrontraj.thetaStartmax >> commonelectrontraj.Startphi >> commonelectrontraj.Ekin >> commonelectrontraj.Xstart >> commonelectrontraj.Ystart >>commonelectrontraj.counter){
	    //cout << endl << "TRAJ: Decay#: " << counter << endl;	    
	    counter ++;
...
//------------------------------------------------------------------------//
	if( commonelectrontraj.ClusterBackDetMC ){
        ...
        cout << endl << "TRAJ: ClusterBackDetMC run" << endl << endl;
	...
	if( commonelectrontraj.PercOn ) thetamax_rad = 30/180.*M_PI;
	else thetamax_rad = 90/180.*M_PI; //angle the particle has at the beginning for gyration radius calculation
	// now we calc the range of values that are allowed in X and Y
	// first, scale the aperture window to DV
	DVwindow_X = ApertX * sqrt_r_A;
	DVwindow_Y = ApertY * sqrt_r_A;
	...
	while ( MonteCarloData >> commonelectrontraj.thetaStartmax >> commonelectrontraj.Startphi >> commonelectrontraj.Ekin >> commonelectrontraj.counter){  
	    counter ++;
		if(commonelectrontraj.thetaStartmax>43)
		{
				if(commonelectrontraj.thetaStartmax>45){
				commonelectrontraj.thetaStartmax=180+commonelectrontraj.thetaStartmax;
				commonelectrontraj.Startphi = 180+commonelectrontraj.Startphi;
	...
	//else cout << "MonteCarloBug: theta > 45" << endl;
			...
			cout<<"Particle Number "<<commonelectrontraj.counter<<" goes to RxB"<<endl;
		}
	} //closing the while loop
