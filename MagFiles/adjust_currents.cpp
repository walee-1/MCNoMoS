void adjust_currents(double R_1, int N_coil, double d_coil, int N_dummy, string inputcoil, string filedir,int iterations, int n_coil_all);

void load_B_field_from_coil_center(double **Bx,double **By, double **Bz, double **B_pos,int i_line, int n_coil_all, string filedir);

#include "small_functions.cpp"          // small functions for programming



void adjust_currents(double R_1, int N_coil, double d_coil, int N_dummy, string inputcoil, string filedir,int iterations, int n_coil_all){

/* The goal is to optimize the current values to optain a goal B-fields
     goals of the B-field could be
        no minimas on the central axis
        homogeneous field on an axis
*/

/* STRUKTUGRAMM:
    Read in the B-field of each coil on the central axis (use function)
    Fast adjusting by changing coil current proportional to the B-field at the coil center
    Calculating the deviation: reduce it by
        testing only one coil (up/down step)
        increasing one coil and decreasing a different coils
        change one coil and the its neighbors in the opposite direction


*/
int coil_number;                 // number of coils of the system

ifstream inputcoil_original;        // original inputcoil.dat file produced by mainmagfield.cpp
inputcoil_original.open(inputcoil.c_str());
if (!inputcoil_original.fail()) {               // extract the coil number
    inputcoil_original >> coil_number;
    } else {coil_number = 98;}


// Storage of the B-field values of each single coil
const int n_datapoints = 2000;
const int n_coil_max = 99;

// B-field line 5:
double **B_5_x = new double*[n_datapoints];             // x-component of the magnetic field for all coils and datapoints at line 5 [T]
    for (int i = 0; i < n_datapoints; i++) B_5_x[i] = new double[n_coil_max];
double **B_5_y = new double*[n_datapoints];             // y-component of the magnetic field for all coils and datapoints at line 5 [T]
    for (int i = 0; i < n_datapoints; i++) B_5_y[i] = new double[n_coil_max];
double **B_5_z = new double*[n_datapoints];             // z-component of the magnetic field for all coils and datapoints at line 5 [T]
    for (int i = 0; i < n_datapoints; i++) B_5_z[i] = new double[n_coil_max];
double **B_5_pos = new double*[n_datapoints];           // position of the magnetic field values at line 5 [m]
    for (int i = 0; i < n_datapoints; i++) B_5_pos[i] = new double[4];
load_B_field_from_coil_center(B_5_x,B_5_y,B_5_z,B_5_pos,5, coil_number, filedir);

cout << B_5_pos[2][3] << endl;
...
// delete used B-field information
for(int i = 0; i < n_datapoints + 1; i++) delete B_5_x[i];          // delete Storage
delete B_5_x;
for(int i = 0; i < n_datapoints + 1; i++) delete B_5_y[i];          // delete Storage
delete B_5_y;
for(int i = 0; i < n_datapoints + 1; i++) delete B_5_z[i];          // delete Storage
delete B_5_z;
...
void load_B_field_from_coil_center(double **Bx,double **By, double **Bz, double **B_pos,int i_line , int n_coil_all, string filedir) {
    int n_data = 2000;
  for(int i_coil = 0; i_coil < n_coil_all; i_coil++){  	// read in all B-field distributions of all coils
	//if(i_coil==n_coil_all && Perc_on == 0){break;}
	// define the input-file path
	string inputfile_name = int_to_string(i_coil + 1);
	//if (i_coil > nrVar-n_adapt+1 && Perc_on == 0){continue;}
	//if(i_coil==nrVar){inputfile_name = "PERConnect";}
	//if(i_coil==nrVar+1){inputfile_name = "PERC";}
	inputfile_name = filedir + "coils/" + inputfile_name + "/coil_center_" + int_to_string(i_line) +  ".txt";

	ifstream inputfile;
	inputfile.open(inputfile_name.c_str(), ios::in);		// open the input-datafile
    if (!inputfile.fail()) {         	// parameters for reading the data files
		// needed variables:
		string trash;
		for (int i_data=0; i_data < 10; i_data++) {inputfile >> trash;} // jump over header
		int j_point= 0;
		int i_data = 0;
		//int k_point = 0;
		double cor_x, cor_y, cor_z, B;
		while(!inputfile.eof()){
			// read in the data line per line
			inputfile >> cor_x >> cor_y >> cor_z >> trash >> B >> Bx[i_data][i_coil] >> By[i_data][i_coil] >> Bz[i_data][i_coil] >> trash;
			// Store the B-field for each datapoint and each coil [T]
			if (i_coil == 0) {
                B_pos[i_data][1] = cor_x;
                B_pos[i_data][2] = cor_y;
                B_pos[i_data][3] = cor_z;
                }
			i_data++;
			}
		inputfile.close();
	  } else {cout << "coil number " << i_coil+1 << " is missing!!"<< endl;
		  for (int i_data=0; i_data < n_data; i_data++) {
			Bx[i_data][i_coil] = 0.;		// descard x & cylindric h
			By[i_data][i_coil] = 0.;		// descard y
			Bz[i_data][i_coil] = 0.;		// descard z
			}
        }
	}
}
